...
def read_data_age_groups(fname, y, percentage_train=0.7, additional_vars=False, waiting=True, hosp=False):
    CATEGORICAL_COLUMNS = [#'FECHA_ACTUALIZACION',
                           # 'ORIGEN',
                           # 'SECTOR',
                           # 'ENTIDAD_UM',
    ...
    age_groups = [(i)*10 for i in range(10)]

    for i in range(len(age_groups)-1):
        row_indexes = df[(df['EDAD'] >= age_groups[i]) & (df['EDAD'] < age_groups[i+1]) ].index
        var_name = 'Age-' +  str(age_groups[i])+'-'+str(age_groups[i+1])
        df[var_name] = 0
        df[var_name][row_indexes] =1
        CATEGORICAL_COLUMNS.append(var_name)
